...
// ─── Build the pre-fill URL for the apply page ───────────────────────────────
//
// Param names here are deliberately chosen to line up 1:1 with the apply-now
// form's own field names (see FormState / jobContext in the apply-now page):
//   currentRole    -> form.currentRole
//   expectedSalary -> form.expectedSalary
//   location       -> form.location
//   skills         -> skills[]
//
// `company` and `jobExperience` describe the JOB POSTING (who's hiring, what
// level they're hiring for) — not the candidate. They're passed through only
// so the apply-now page can render its "Applying for X at Y" context banner.
// They are intentionally NOT named `currentCompany` / `experience`, because
// those apply-now fields describe the candidate's own background and must
// never be silently overwritten by details of the job they're applying to.
const buildApplyHref = (job: Job) => {
...
function JobCard({ job, saved, onSave, view, index }: JobCardProps) {
  ...
  <div className="flex-shrink-0 self-center">
    <Link
      href={buildApplyHref(job)}
      onClick={(e) => e.stopPropagation()}
      className="inline-block text-[13px] font-semibold text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-xl shadow-sm shadow-blue-200 transition-all hover:-translate-y-0.5 whitespace-nowrap"
    >
      Apply Now
    </Link>
  </div>
  ...
  <Link
    href={buildApplyHref(job)}
    onClick={(e) => e.stopPropagation()}
    className="mt-4 block w-full text-center text-[13px] font-semibold text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white py-2.5 rounded-xl transition-all duration-200 border border-blue-100 hover:border-blue-600 hover:shadow-sm"
  >
    Apply Now →
  </Link>
)}
